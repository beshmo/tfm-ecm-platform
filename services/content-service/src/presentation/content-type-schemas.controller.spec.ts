...
describe("content-service schema endpoints", () => {
...
describe("content-service schema administration authorization", () => {
  let app: INestApplication;
  ...
  const NON_ADMIN = "folder:read,generic:read";
  const ADMIN = "content-type:read,content-type:create,content-type:update,content-type:delete";

  it("GIVEN a non-admin WHEN schema administration is attempted THEN it is forbidden", async () => {
    await request(app.getHttpServer())
      .get("/api/management/content-types/definitions")
      .set("x-ecmp-permissions", NON_ADMIN)
      .expect(403);
    ...
    await request(app.getHttpServer())
      .put("/api/management/content-types/generic/versions/1.0")
      .set("x-ecmp-permissions", NON_ADMIN)
      .send({ schemaSource: schemaSource("generic", "1.0") })
      .expect(403);

    await request(app.getHttpServer())
      .delete("/api/management/content-types/generic/versions/1.0")
      .set("x-ecmp-permissions", NON_ADMIN)
      .expect(403);

    await request(app.getHttpServer())
      .post("/api/management/content-types/generic/move")
      .set("x-ecmp-permissions", NON_ADMIN)
      .send({ targetFolderId: "FLD-system-schemas" })
      .expect(403);
  });

  it("GIVEN an administrator WHEN schema administration is attempted THEN it is allowed", async () => {
    await request(app.getHttpServer())
      .get("/api/management/content-types/definitions")
      .set("x-ecmp-permissions", ADMIN)
      .expect(200)
      .expect((response) => {
        expect(response.body).toEqual([
          expect.objectContaining({ name: "generic", folderId: "FLD-system-schemas" })
        ]);
      });
  ...
  it("GIVEN a non-admin WHEN active schemas are consumed THEN authoring access remains available", async () => {
    await request(app.getHttpServer())
      .get("/api/management/content-types")
      .set("x-ecmp-permissions", NON_ADMIN)
      .expect(200)
