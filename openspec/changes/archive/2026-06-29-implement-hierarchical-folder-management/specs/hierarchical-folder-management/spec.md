...
The system SHALL require user-created folder names to be trimmed, non-empty, not `.` or `..`, free of path separators, free of control characters, and free of common unsafe filesystem symbols `<`, `>`, `:`, `"`, `|`, `?`, and `*`.
...
The system SHALL delete only non-root folders that have no child folders and no assigned content.
...
- **WHEN** a user deletes a folder that has assigned content
- **THEN** the system rejects the operation as a non-empty folder conflict
