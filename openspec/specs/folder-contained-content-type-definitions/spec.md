...
Define content type definitions as folder-contained repository objects stored under the protected `/system/schemas` namespace, including administrator-only schema folder organization and content type definition move behavior. Content Service owns these objects; each definition groups its schema versions and carries a repository folder location.
