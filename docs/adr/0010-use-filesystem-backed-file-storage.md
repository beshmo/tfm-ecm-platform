...
The first implementation needs to store binary documents while keeping metadata in MongoDB. Object storage is not required for the initial academic phase.

## Decision

ECMP will use filesystem-backed file storage for binary files.

MongoDB will store document metadata and internal storage paths. Management and Delivery file storage will be separated.
